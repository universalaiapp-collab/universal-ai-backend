...
export async function geminiChat(params: GeminiChatParams): Promise<GeminiResponse> {
  ...
  try {
    ...
    if (typeof data === "string") {
      ...
      output = Array.isArray(data.candidates)
        ? data.candidates.map((c: any) => c.content ?? c).join("\n")
        : String(data.candidates);
    } else if (data?.output?.[0]?.content) {
      // another possible structure
      output = data.output.map((o: any) => o.content?.map((c: any) => c.text ?? c).join("") ?? JSON.stringify(o)).join("\n");
    } else if (data?.result) {
