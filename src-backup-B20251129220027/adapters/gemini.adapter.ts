...
declare const fetch: any; // Node 18 has global fetch, TS may not include dom types â€” keep this to avoid errors
...
function buildUrl(model: string, useKey = false) {
...
async function getAccessToken() {
  ...
  if (!token) throw new Error('Unable to obtain access token from Google ADC');
  return token;
}
 ...
 */
export async function callGemini(prompt: string, opts: any = {}) {
  if (!prompt) throw new Error('gemini.adapter: missing prompt');
  ...
  const normalized: any = { choices: [], usage: null, raw: json };
    ...
    if (json?.candidates && Array.isArray(json.candidates)) {
      ...
      for (const out of json.outputs) {
        const text =
          ...
          (out?.items && out.items.map((it: any) => it?.text || '').join('')) ||
          '';
  ...
  if (json?.usage) normalized.usage = json.usage;
  else if (json?.metadata?.tokenCount) normalized.usage = { total_tokens: json.metadata.tokenCount };
