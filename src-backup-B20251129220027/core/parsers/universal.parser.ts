...
function joinPieces(arr: any[]): string {
  return arr
    .map((c) => {
...
export function parseGeminiChat(raw: any): UniversalParsed {
  try {
    if (!raw) {
    ...
    if (Array.isArray(raw.outputs) && raw.outputs.length > 0) {
      const texts = raw.outputs.map((o: any) => {
        if (typeof o === "string") return o;
        if (o?.content && Array.isArray(o.content)) return joinPieces(o.content);
...
export function parseOpenAIChat(raw: any): UniversalParsed {
  try {
    if (!raw) return { success: false, provider: "openai", type: "text", output: "", raw };
...
export function parseEmbedding(provider: string, raw: any): UniversalParsed {
  try {
    // OpenAI-style embeddings: data[0].embedding
    if (raw?.data && Array.isArray(raw.data) && raw.data[0]?.embedding) {
      return { success: true, provider, type: "embedding", output: raw.data.map((d: any) => d.embedding), raw };
    }
...
export function parseUniversalResponse(provider: string, raw: any): UniversalParsed {
  ...
  switch (p) {
    ...
    default:
      ...
      if (raw == null) return { success: false, provider, type: "unknown", output: "", raw };
      if (typeof raw === "string") return { success: true, provider, type: "text", output: raw, raw };
      if (typeof raw === "object") {
        // if object has message/text/response fields prefer them
        if (typeof raw?.text === "string") return { success: true, provider, type: "text", output: raw.text, raw };
        if (typeof raw?.response === "string") return { success: true, provider, type: "text", output: raw.response, raw };
        if (typeof raw?.message === "string") return { success: true, provider, type: "text", output: raw.message, raw };
        // finally, stringify
