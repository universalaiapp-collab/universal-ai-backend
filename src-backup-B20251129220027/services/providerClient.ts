...
async function callProvider({ provider, model, prompt, timeoutMs = 12000 }: CallArgs): Promise<any> {
  // If OPENAI_API_KEY present and provider indicates openai, call the real API.
  const openaiKey = process.env.OPENAI_API_KEY;
  if (provider === "openai" && openaiKey) {
    return withTimeout(callOpenAIChat({ apiKey: openaiKey, model, prompt }), timeoutMs);
  }

  // If no real key (or provider not openai), fall back to simulated provider to avoid breaking local dev.
  return withTimeout(_callSimulatedProvider(provider, model, prompt), timeoutMs);
}
 ...
 */
async function callOpenAIChat({ apiKey, model, prompt }: { apiKey: string; model: string; prompt: string }) {
 ...
 */
async function _callSimulatedProvider(provider: string, model: string, prompt: string) {
  ...
  if (env[failKey] && env[failKey]?.toLowerCase() === "true") {
    const err: any = new Error(`${provider} simulated failure`);
