...
async function score(provider: string, model: string, prompt: string, opts?: any): Promise<number> {
  ...
  if (circuitBreaker.isOpen(provider)) {
    ...
    const failureRate = circuitBreaker.getFailureRate(provider); // 0..1
  ...
  if (p.includes("image:") || p.includes("generate image")) {
    if (model.toLowerCase().includes("image")) score += 10;
    else score -= 10;
  }
