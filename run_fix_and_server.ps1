# ---------------------------
# Elevated helper script
# Run inside elevated PowerShell (Run as Administrator)
# Stops target PID, frees port 4000, starts server in new window, tests endpoint, keeps window open.
# ---------------------------

$ErrorActionPreference = "Stop"

# Use a name that does not collide with built-in variables
$targetPid = 16504
$port = 4000

Write-Host "=== Elevated helper started ===" -ForegroundColor Cyan
...
try {
  ...
  try {
    ...
    Write-Host "taskkill succeeded for PID $($targetPid)." -ForegroundColor Green
  } catch {
...
function WaitForOpen([string]$host='localhost',[int]$port=4000,[int]$timeoutSec=30) {
...
if (-not (WaitForOpen -host 'localhost' -port $port -timeoutSec 30)) {
