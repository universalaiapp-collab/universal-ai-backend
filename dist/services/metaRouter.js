...
const modelScorer_1 = __importDefault(require("./modelScorer"));
...
async function choose(prompt, opts) {
    ...
    const scored = [];
    for (const c of raw) {
        const score = await modelScorer_1.default.score(c.provider, c.model, prompt, opts);
    ...
    if (opts?.hint) {
        ...
        scored.sort((a, b) => {
            ...
            if (aHint !== bHint)
                return bHint - aHint; // bring hinted ones earlier
